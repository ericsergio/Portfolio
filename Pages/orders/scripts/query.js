...
function placeControls(that) {
  let dist = $(that)
    .attr("id")
    .substr($(that).attr("id").length - 2);
  $(that).addClass("verify");
  console.log(that);
  if ($("button").length < 1) {
    $(".results").before(
      `<button id = "checkOrder" onclick="apiQueryCheckSetup('${dist}')">Check Order</button>`
    );
  }
}
...
function sendOrder() {
  ...
  for (p in urlItems) {
    $.get(urlItems[p], function (data) {
      ...
      $("#itemizedBody").append(
        `<tr id = ${rowId}><td class = 'data'>${name}</td><td class = 'data'>$${price}</td></tr>`
      );
      counter++;
    });
  }
  ...
  $(".itemized").before(
    `<button id = "OkBtn" onclick = "doHideMsg()">OK</button>`
  );
  $(".testResult").fadeIn(700);
}
...
function doTotal() {
  setTimeout(function () {
    $.get("http://localhost:3000/total", function (data) {
      ...
      console.log(total);
      $(".itemized").before(
        `<h2 id = 'totalLbl'>Invoice Total</h2><h3 id = 'InvoiceTotal'>$${total}</h3>`
      );
    });
  }, 1800);
}
